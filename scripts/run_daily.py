...
適用の目安: やや柔軟に判断し、投稿の50-60%程度に含まれることを想定してください。"""
            ...
            # 出力フォーマットのテンプレート（共通部分）
            output_format = f"""
企業名、「製品/機能名」を発表/リリース

[1-2文でサマリ。何が新しいのか、なぜ重要なのか]
...
∙ 対象ユーザー: [具体的に]
∙ 提供開始: [いつから]
∙ 業界への影響: [ビジネス効果や技術的優位性]

【重要な制約】
- 各見出しの下に3-6個の詳細項目を記載
- 箇条書きには「∙」（U+2219、BULLET OPERATOR）のみ使用
- 見出しはシンプルな平文（セクション番号なし、太字なし）
- URLはサマリの直後に配置
- 全体で600-800文字程度（Xスレッドとして適切な長さ）
- 見出しは記事内容に応じて柔軟に変更可能
- タイトルから推測して具体的に記述してください
- 「核心」「革新的」「画期的」などのAIっぽい表現は使わない"""
